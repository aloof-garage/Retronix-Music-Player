    ...
    <div style={{ flex: 1, overflow: 'auto', padding: '20px' }}>
      ...
      {displayQueue.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 60, gap: 12 }}>
          ...
          <div style={{ fontSize: 12, color: T.textMuted }}>Queue is empty — play a track or album to fill it</div>
        </div>
      ) : (
        <div style={{ borderRadius: 10, overflow: 'hidden', boxShadow: T.neumorphIn, background: T.surfaceDeep }}>
          {displayQueue.map((track, i) => {
            ...
            return (
              <div
                key={`${track.id}-${i}`}
                onDoubleClick={() => playTrack(track, displayQueue)}
                style={{
              ...
              >
                <span style={{ fontSize: 10, color: isActive ? T.accent : T.textMuted, width: 24, textAlign: 'center', flexShrink: 0 }}>
                  {isActive && isPlaying ? '▶' : i + 1}
                </span>
                <AlbumArt color={track.color} size={32} trackId={track.id} artworkPath={track.artwork_path}/>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, color: isActive ? T.accent : T.text, fontWeight: isActive ? 700 : 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {track.title}
                  </div>
