  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, WebkitAppRegion: 'no-drag' }}>
        ...
        <span style={{ fontSize: 13, letterSpacing: '0.2em', fontWeight: 700, color: T.accent }}>
          RETRONIX
        </span>
        <span style={{ fontSize: 10, color: T.textMuted, letterSpacing: '0.15em' }}>MK·II</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, WebkitAppRegion: 'no-drag' }}>
        ...
        <WindowBtn onClick={() => window.electronAPI?.window.close()} color="#ff5f56" isClose>✕</WindowBtn>
      </div>
    </div>
  )
}

function WindowBtn({ children, onClick, color, isClose }) {
  ...
  return (
    <button
      ...
      style={{
        ...
        cursor: 'pointer', background: hover ? color : 'rgba(255,255,255,0.15)',
